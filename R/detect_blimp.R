...
detect_blimp <- function() {
    ...
    if (grepl('darwin', user_os))       return(detect_blimp_macos(exec))
    else if (grepl('linux', user_os))   return(detect_blimp_linux(exec))
    else if (grepl('windows', user_os)) return(detect_blimp_windows(paste0(exec,'.exe')))
