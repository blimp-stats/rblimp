...
simplehist_wrapper_list <- function(x, round = TRUE, xlab = "", ylab = "Frequency", off = 0.0, lwd = 3,
    ...
    maxX <- max(sapply(x, function(i) {
        max(i)
    }))
    ticksX <- unlist(lapply(x, function(i) {
        as.numeric(attr(i, "dimnames")[[1]])
    }))
    if (is.null(ticks)) {
        ticks <- pretty(ticksX)
...
setGeneric(
    ...
    function(x, param, round = TRUE, ylab = "Frequency", off = 0.0, lwd = 3,
        ...
        if (class(x) == "table") {
            ...
            return(simplehist_wrapper(x,
                round = round, ylab = ylab, off = off, lwd = lwd,
                col = col, plot = plot, ...
            ))
        }
    }
...
setMethod(
    "simplehist", "blimp_obj",
    function(x, param, round = FALSE, ylab = "Frequency", off = 0.0, lwd = 3,
             col = c("black", "red"), plot = TRUE, ...) {
        Y <- simplehist_wrapper(x@iterations[, param],
            round = round, ylab = ylab, off = off, lwd = lwd,
            col = col, plot = plot, ...
        )
        ...
        if (plot) {
            abline(
                v = x@estimates[param, 3:4], # Add 95% intervals
