...
setGeneric("compare",function(model0, model, use = 'mean', greaterThan = TRUE, suffixes =
                       c('R2: Fixed Effects ', 'R2: Level-2 Random Intercepts ',
...
#' @export
setMethod("show", "blimp_cp",
function( object ) {
    if(object@greaterThan) cat("Proportion of parameter greater than:\n")
    else  cat("Proportion of parameter less than:\n")
    show(object@table)
})
...
get_value <- function(x, use) {
    ...
    # Parse character
    if (is.character(use)) {
        if (use == 'mean')         return( mean(x)   )
        else if ( use == 'median') return( median(x) )
        else {
    ...
    # Return Function
    else if (is.function(use))  return(use(x))
    # Return value
    if (use < 1) return(quantile(x, probs = use))
    else          return(quantile(x, probs = use / 100))
}
...
model_compare_wrapper <- function(suffix, model, model0, use, greaterThan) {
    ...
    for (i in seq_along(mnames)) {
        if (matches[i]) {
            ...
            if (greaterThan) output[[count]] <- c(mean(model@iterations[,mnames[i]] > value))
            else  output[[count]] <- c(mean(model@iterations[,mnames[i]] < value))
...
#' @export
setMethod("compare", signature(model = "blimp_obj", model0 = "blimp_obj"),
          function(model0, model, use = 'mean', greaterThan = TRUE, suffixes =
              ...
              for(i in seq_along(use)) {
                  ...
                  if (use_char == 'mean')  colnames(mat) <- 'mean'
                  else if ( use_char == 'median')  colnames(mat) <- 'median'
                  else {
                      val <- suppressWarnings(as.numeric(use_char))
                      if (is.na(val)) {
                          colnames(mat) <- use_char
                      } else {
                          if (val < 1) colnames(mat) <- paste0(val*100, '%')
                          else colnames(mat) <- paste0(val, '%')
                      }
                  }
                  if (i == 1) output <- mat
                  else output <- cbind(output,mat)
              }
              # Convert to object
              output <- new("blimp_cp",
                            table = output,
