...
format_show <- function (x, digits) {
    r <- as.data.frame(lapply(1:length(x), function(i) {
        if (class(x[[i]]) != "character") {
            ...
            else round(x[[i]], digits["default__"])
        }
        else {
            return(x[[i]])
        }
